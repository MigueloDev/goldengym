...
export default function EditPlan({ plan }: Props) {
  const { data, setData, put, processing, errors } = useForm({
    ...
    renewal_period_days: plan.renewal_period_days.toString(),
    status: plan.status,
    features: plan.features || [],
    subscription_price_usd: plan?.subscription_price_usd?.toString() || '',
    subscription_price_local: plan?.subscription_price_local?.toString() || '',
  });
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="space-y-2">
                    <Label htmlFor="price_usd">Precio Inscripción en USD *</Label>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        step="0.01"
                        min="0"
                        value={data.subscription_price_usd}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setData('subscription_price_usd', e.target.value)}
                        placeholder="0.00"
                        className={`pl-10 ${errors.subscription_price_usd ? 'border-red-500' : ''}`}
                      />
                    </div>
                    {errors.subscription_price_usd && (
                      <p className="text-sm text-red-500 flex items-center gap-1">
                        <AlertCircle className="h-4 w-4" />
                        {errors.subscription_price_usd}
                      </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        step="0.01"
                        min="0"
                        value={data.subscription_price_local}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setData('subscription_price_local', e.target.value)}
                        placeholder="0.00"
                        className={`pl-10 ${errors.subscription_price_local ? 'border-red-500' : ''}`}
                      />
                    </div>
                    {errors.subscription_price_local && (
                      <p className="text-sm text-red-500 flex items-center gap-1">
                        <AlertCircle className="h-4 w-4" />
                        {errors.subscription_price_local}
                      </p>
